...
import AddOrUpdate        from './achievement-type-add-or-update'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      achievementTypeApi.fuzzy(Object.assign({}, this.dataForm, {
    ...
    deleteHandle (row) {
      ...
      this.$confirm(`确定对[${rows.map(item => item.name).join(',')}]进行[${row ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        achievementTypeApi.batchRemove(rows).then(({ data }) => {
          if (data.data) {
            this.$notify.success({
              message: '删除成功',
              title: '删除'
