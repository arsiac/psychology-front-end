...
import AddOrUpdate    from './subject-type-add-or-update'

export default {
  data () {
    return {
      authUrl: 'base/subject/type',
      dataForm: {
        name: ''
      },
      dataList: [],
  ...
  methods: {
    ...
    getDataList () {
      ...
      subjectTypeApi.fuzzy(Object.assign({}, this.dataForm, {
    ...
    deleteHandle (row) {
      ...
      this.$confirm(`确定对[${rows.map(item => item.name).join(',')}]进行[${row ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        subjectTypeApi.remove(rows).then(({ data }) => {
          if (data.data) {
            this.$notify.success({
              message: '删除成功',
              title: '删除'
