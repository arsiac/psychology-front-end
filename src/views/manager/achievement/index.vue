  ...
  <div class="mod-config">
    ...
    <el-table
      ...
      <el-table-column
          ...
          label="获奖时间">
        <template slot-scope="scope">
          <span v-if="scope.row.date">{{ $dateString(scope.row.date) }}</span>
        </template>
      </el-table-column>
      <el-table-column
          prop="code"
...
import Audit from './achievement-audit'

export default {
  data () {
    return {
      authUrl: 'manager/achievement',
      dataForm: {
        name: ''
      },
      dataList: [],
  ...
  methods: {
    ...
    getDataList () {
      ...
      achievementApi.fuzzy(Object.assign({}, this.dataForm, {
    ...
    deleteHandle (row) {
      ...
      this.$confirm(`确定对[${rows.map(item => item.name).join(',')}]进行[${row ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        achievementApi.batchRemove(rows).then(({ data }) => {
          if (data.data) {
            this.$notify.success({
              message: '删除成功',
              title: '删除'
