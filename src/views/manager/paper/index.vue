  ...
  <div class="mod-config">
    ...
    <el-table
      ...
      <el-table-column
          ...
          label="第一作者">
        <template slot-scope="scope">
          <span v-if="scope.row.firstAuthorName">{{ scope.row.firstAuthorName }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column
          prop="correspondingAuthorName"
          header-align="center"
          align="center"
          label="通讯作者">
        <template slot-scope="scope">
          <span v-if="scope.row.correspondingAuthorName">{{ scope.row.correspondingAuthorName }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column
...
import Audit from './paper-audit'

export default {
  data () {
    return {
      authUrl: 'manager/paper',
      dataForm: {
        name: ''
      },
      dataList: [],
  ...
  methods: {
    ...
    getDataList () {
      ...
      paperApi.fuzzy(Object.assign({}, this.dataForm, {
    ...
    deleteHandle (row) {
      ...
      this.$confirm(`确定对[${rows.map(item => item.name).join(',')}]进行[${row ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        paperApi.batchRemove(rows).then(({ data }) => {
          if (data.data) {
            this.$notify.success({
              message: '删除成功',
              title: '删除'
